...
class MyApp:
    ...
    def run(self, host="0.0.0.0", port="8050"):
        ...
        def update_content(n_intervals, selected_part, start_date, end_date, view):
            ...
            #     daily_avg = filtered_df.groupby(filtered_df['full_timestamp'].dt.date)['kensaTime'].mean()
            #     daily_avg_fig = go.Figure(go.Scatter(
            #         x=daily_avg.index, y=daily_avg, mode='lines+markers', name='Daily Average KensaTime'
            #     ))
            #     daily_avg_fig.update_layout(title="日常平均検査時間", xaxis_title="日付", yaxis_title="時間(秒)")
            #     per_entry_fig = go.Figure(go.Scatter(
            #         x=filtered_df['full_timestamp'], y=filtered_df['kensaTime'],
            #         mode='markers', name='Per Entry KensaTime'
            #     ))
            #     per_entry_fig.update_layout(title="各時検査時間", xaxis_title="日付", yaxis_title="時間(秒)")
            #     return html.Div([
            #         dcc.Graph(figure=daily_avg_fig),
            #         dcc.Graph(figure=per_entry_fig)
            #     ])
            ...
            #     daily_avg_df = expanded_pitch_df.groupby('Date').mean().reset_index()
            #     daily_std_df = expanded_pitch_df.groupby('Date').std().reset_index()
            #     valid_std_mask = daily_std_df.iloc[:, 1:].notna().any(axis=1)
            #     daily_avg_df = daily_avg_df[valid_std_mask]
            #     daily_std_df = daily_std_df[valid_std_mask]
            #     if daily_avg_df.empty:
            #         return html.Div("AIKENSAデータはありません。", style={'color': 'red', 'font-weight': 'bold'})
            #     daily_avg_df.columns = ['Date'] + [f'Pitch {i+1}' for i in range(actual_pitch_count)]
            #     daily_std_df.columns = ['Date'] + [f'Pitch {i+1}_std' for i in range(actual_pitch_count)]
            #     pitch_graphs = []
            #     for i in range(actual_pitch_count):
            #         pitch_column = f'Pitch {i+1}'
            #         std_column = f'{pitch_column}_std'
            #         nominal = nominal_pitch[i] if i < len(nominal_pitch) else None
            #         tol = tolerance[i] if i < len(tolerance) else None
            #         fig = go.Figure()
            #         fig.add_trace(go.Scatter(
            #             x=daily_avg_df['Date'], y=daily_avg_df[pitch_column],
            #             mode='lines+markers', name=pitch_column
            #         ))
            #         upper_3sigma = daily_avg_df[pitch_column] + 3 * daily_std_df[std_column]
            #         lower_3sigma = daily_avg_df[pitch_column] - 3 * daily_std_df[std_column]
            #         fig.add_trace(go.Scatter(
            #             x=daily_avg_df['Date'], y=upper_3sigma,
            #             mode='lines', line=dict(color='rgba(0, 128, 0, 0)'),
            #             showlegend=False
            #         ))
            #         fig.add_trace(go.Scatter(
            #             x=daily_avg_df['Date'], y=lower_3sigma,
            #             mode='lines', fill='tonexty', fillcolor='rgba(0, 128, 0, 0.2)',
            #             line=dict(color='rgba(0, 128, 0, 0)'), showlegend=True,
            #             name='±3σ範囲'
            #         ))
            #         if nominal is not None and tol is not None:
            #             fig.add_trace(go.Scatter(
            #                 x=daily_avg_df['Date'],
            #                 y=[nominal] * len(daily_avg_df),
            #                 mode='lines',
            ...
            #             fig.add_trace(go.Scatter(
            #                 x=daily_avg_df['Date'],
            #                 y=[nominal + tol] * len(daily_avg_df),
            #                 mode='lines',
            ...
            #     for i in range(actual_pitch_count):
            #         pitch_column = f'Pitch {i+1}'
            #         # Subset the data for this pitch and Date, and filter out zeros.
            #         temp_df = expanded_pitch_df[[pitch_column, 'Date']].copy()
            #         temp_df = temp_df[temp_df[pitch_column] != 0]
            ...
            #         # Compute the mean and standard deviation from the filtered data.
            #         if not temp_df.empty:
            #             mean_val = temp_df[pitch_column].mean()
            #             std_val = temp_df[pitch_column].std()
                        
            #             # Overlay a horizontal line for the mean.
            #             fig.add_shape(
            #                 type="line",
            #                 xref="paper", x0=0, x1=1,
            #                 yref="y", y0=mean_val, y1=mean_val,
            #                 line=dict(color="black", dash="solid"),
            #                 name="Mean"
            #             )
            #             # Overlay horizontal lines for ±3σ.
            #             fig.add_shape(
            #                 type="line",
            #                 xref="paper", x0=0, x1=1,
            #                 yref="y", y0=mean_val - 3*std_val, y1=mean_val - 3*std_val,
            ...
            #         # Optionally, if you want to also show the nominal value and tolerance boundaries:
            #         nominal = nominal_pitch[i] if i < len(nominal_pitch) else None
            #         tol = tolerance[i] if i < len(tolerance) else None
            #         if nominal is not None:
            #             fig.add_shape(
